...
export default function Navbar() {
  ...
  return (
    <>
      <header
        className={`fixed top-0 inset-x-0 z-[100] transition-all duration-500 ease-in-out ${isVisible ? 'translate-y-0' : '-translate-y-full'
          } ${isAtTop
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  to={link.path}
                  className={`text-sm font-medium transition-colors duration-200 focus:outline-none ${isActive
                      ? "text-white"
      ...
      <div
        className={`fixed inset-0 z-[90] bg-[#030303]/95 backdrop-blur-2xl transition-all duration-500 flex flex-col items-center justify-center ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
          }`}
      >
          ...
          {navLinks.map((link, index) => {
            ...
            return (
              <Link
                ...
                className={`text-2xl md:text-3xl font-semibold tracking-tight transition-all duration-300 ${isActive ? "text-white" : "text-zinc-500 hover:text-white"
                  }`}
              >
                {link.name}
              </Link>
